...
def replace_layers(model, old, new):
    for n, module in model.named_children():
        ...
        if isinstance(module, old):
            ## simple module
            input_size, hidden_size = (int(x.strip()) for x in module.extra_repr().split(',')[0:2])
            new_layer = new(input_size, hidden_size)
            with torch.no_grad():
                for i in range(len(new_layer._flat_weights)):
                    new_layer._flat_weights[i].data = module._flat_weights[i].data
            new_layer.__update__()
...
def lstm_quan():
    ...
    replace_layers(model, torch.nn.LSTM, quan_LSTM)
    return model